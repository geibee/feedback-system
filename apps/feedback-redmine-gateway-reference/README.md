...
`FEEDBACK_PUBLIC_ORIGIN`へSPAの正確な公開originを指定します。profileは次のどちらか一方で渡します。

- `FEEDBACK_REDMINE_GATEWAY_PROFILE_FILE`: read-only server profile JSONのabsolute path。`clientProfileRef`は同じdirectoryからのrelative pathまたはabsolute path
- `FEEDBACK_REDMINE_GATEWAY_PROFILE_JSON`: `clientProfileRef`の代わりに`clientProfile` objectを含むJSON文字列。環境変数だけで配備するcontainer platform向け

`FEEDBACK_REDMINE_GATEWAY_API_KEY`または
