...
`RedmineFeedbackPort`と端末内`ClientStatePort`を中心に構成する共通React UIです。旧`@geibee/react`の2つの起動ボタン、
...
DOM captureは`@geibee/dom-capture`を既定で接続し、共通target解決は`@geibee/react-ui`を使用します。Host Adapterの
`captureEvidence`は任意の上書きであり、未指定でもキャプチャします。保存画像には選択位置のFeedbackピンを焼き込みます。
legacyの`@geibee/react`には依存しません。
